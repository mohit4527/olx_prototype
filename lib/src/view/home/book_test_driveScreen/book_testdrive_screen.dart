...
class TestDriveScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final controller = Get.put(TestDriveController(carId));

    return Scaffold(
      appBar: AppBar(
        backgroundColor: AppColors.appGreen,
        centerTitle: true,
        title: Text("User Test Drives - $productTitle", style: const TextStyle(color: AppColors.appWhite)),
        leading: IconButton(
          onPressed: () => Get.back(),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (item.phone != null)
                        Row(
                          children: [
                            const Icon(Icons.phone, size: 18, color: AppColors.appGreen),
                      ...
                      if (item.date != null)
                        Row(
                          children: [
                            const Icon(Icons.calendar_today, size: 18, color: AppColors.appGreen),
                            const SizedBox(width: 6),
                      ...
                      if (item.time != null)
                        Row(
                          children: [
                            const Icon(Icons.access_time, size: 18, color: AppColors.appGreen),
                            const SizedBox(width: 6),
                      ...
                      if (status == "pending")
                        Row(
                          children: [
                            ElevatedButton(
                              onPressed: () => controller.updateTestDriveStatus(index, "accept"),
                              style: ElevatedButton.styleFrom(backgroundColor: Colors.green),
                              child: const Text("Accept", style: TextStyle(color: AppColors.appWhite)),
                            ),
                            const SizedBox(width: 12),
                            ElevatedButton(
                              onPressed: () => controller.updateTestDriveStatus(index, "reject"),
